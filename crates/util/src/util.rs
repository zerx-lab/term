...
pub fn get_zed_cli_path() -> Result<PathBuf> {
    ...
    let possible_locations: &[&str] = if cfg!(target_os = "macos") {
        ...
        // bin/zterm.exe is for current installed builds, bin/zed.exe is for older installed
        // builds before the zterm rename, ./cli.exe is for development builds.
        &["bin/zterm.exe", "bin/zed.exe", "./cli.exe"]
