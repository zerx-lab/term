//! Paths to locations used by zterm.
...
/// On macOS, this is `~/Library/Application Support/Zterm`.
/// On Linux/FreeBSD, this is `$XDG_DATA_HOME/zterm`.
/// On Windows, this is `%LOCALAPPDATA%\Zterm`.
static CURRENT_DATA_DIR: OnceLock<PathBuf> = OnceLock::new();

/// The resolved config directory, combining custom override or platform defaults.
/// This is set once and cached for subsequent calls.
/// On macOS, this is `~/.config/zterm`.
/// On Linux/FreeBSD, this is `$XDG_CONFIG_HOME/zterm`.
/// On Windows, this is `%APPDATA%\Zterm`.
...
pub fn config_dir() -> &'static PathBuf {
    CONFIG_DIR.get_or_init(|| {
        if let Some(custom_dir) = CUSTOM_DATA_DIR.get() {
            ...
            home_dir().join(".config").join("zterm")
        }
    })
}
...
pub fn data_dir() -> &'static PathBuf {
    CURRENT_DATA_DIR.get_or_init(|| {
        if let Some(custom_dir) = CUSTOM_DATA_DIR.get() {
            ...
            home_dir().join("Library/Application Support/Zterm")
        } else if cfg!(any(target_os = "linux", target_os = "freebsd")) {
...
pub fn state_dir() -> &'static PathBuf {
    ...
    STATE_DIR.get_or_init(|| {
        if cfg!(target_os = "macos") {
            return home_dir().join(".local").join("state").join("Zterm");
        }
...
pub fn temp_dir() -> &'static PathBuf {
    ...
    TEMP_DIR.get_or_init(|| {
        ...
        home_dir().join(".cache").join("zterm")
    })
}
...
pub fn logs_dir() -> &'static PathBuf {
    ...
    LOGS_DIR.get_or_init(|| {
        if cfg!(target_os = "macos") {
            home_dir().join("Library/Logs/Zterm")
        } else {
            data_dir().join("logs")
        }
    })
...
/// Returns the path to the `zterm.log` file.
pub fn log_file() -> &'static PathBuf {
    static LOG_FILE: OnceLock<PathBuf> = OnceLock::new();
    LOG_FILE.get_or_init(|| logs_dir().join("zterm.log"))
}

/// Returns the path to the `zterm.log.old` file.
pub fn old_log_file() -> &'static PathBuf {
    static OLD_LOG_FILE: OnceLock<PathBuf> = OnceLock::new();
    OLD_LOG_FILE.get_or_init(|| logs_dir().join("zterm.log.old"))
}
